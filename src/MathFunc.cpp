...
/*==============================================
Get2DBezier:
2次ベジェ曲線の任意の段階における(x,y)を求める。
---
StartX, StartY: 最初の(x,y)
CtrlX,CtrlY: 制御点(X,Y)
DivNum,NowPhase: 分割数と現在の段階
(0<=NowPhase<=DivNum、但しDivNum>0)
DivNumが0だった場合や、
NowPhase>DivNumだった場合は全て0を返す。
---
RtnX,RtnY: 返り値(x,y)
RtnV: 返り値。Vは(X,Y)から両端への距離。
RtnRadian: 返り値。角度(Radian)
=============================================*/
void Get2DBezier(double StartX, double StartY,
	...
	double hoge1;
	double hoge2;
	Get2DBezier(StartX,StartY,EndX,EndY,CtrlX,CtrlY,DivNum,NowPhase,RtnX,RtnY,&hoge1,&hoge2);
}
...
void Get2DBezier(double StartX, double StartY,
	...
	for(int i=0;i<2;i++){ //上端と下端計算
		//2次ベジェ曲線本体
		u = ( 1.0 / (DivNum+1) ) * (NowPhase+(1-i));
		psm[0]=(1-u)*StartX+u*CtrlX;
		psm[1]=(1-u)*StartY+u*CtrlY;
		pme[0]=(1-u)*CtrlX+u*EndX;
		pme[1]=(1-u)*CtrlY+u*EndY;
		pse[0]=(1-u)*psm[0]+u*pme[0];
		pse[1]=(1-u)*psm[1]+u*pme[1];
		Edge[i][0]=pse[0];
		Edge[i][1]=pse[1];
	}
	double x,y;
	x=(Edge[0][0]+Edge[1][0])*0.5;
	y=(Edge[0][1]+Edge[1][1])*0.5;
	*RtnX=x;
	*RtnY=y;
	*RtnRadian=myatan2(Edge[0][1]-Edge[1][1],Edge[0][0]-Edge[1][0]); //角度算出
	*RtnV=sqrt(pow((x-Edge[0][0]),2)+pow((y-Edge[0][1]),2)); //距離算出
	return;
 }
...
int CheckHitRotaRect(int x, int y, int cx, int cy, double angle, int width, int height){
	...
	int px=x-cx;
	int py=y-cy;
	double px2=px*c+py*s;
	double py2=-1*px*s+py*c;
	if(abs(2*px2)<=width && abs(2*py2)<=height){
		return 1;
	}else{
		return 0;
	}
...
int NumDividable(const int NumDivided,const int NumDivideBy){
	int r=(int)((NumDivided-(NumDivided%NumDivideBy))/NumDivideBy);
	return r;
}
...
double mycos(double rad){
	return mysin(rad+RIGHT_ANGLE);
}
float mycos(float rad){
	return mysin(rad+RIGHT_ANGLE_F);
}
...
float myatan2(float y, float x){
	return (float)(myatan2((double)x,(double)y));
}
...
//度をラジアンに変換
double math::ToRadian(double Degrees){
...
//当たり判定
int math::hitcheck(double x1,double y1, int Width1,int Height1,
				   double x2,double y2, int Width2,int Height2){
	if( ((Width1+Width2)*0.5 >= abs(x2-x1)) && ((Height1+Height2)*0.5 >= abs(y2-y1)) ){
		return 1;
	}else{
		return 0;
	}
...
//ベクトル成分からラジアンに変換
double math::vec2rad(const double& vx, const double& vy){
	if(vx==0&&vy==0){
		return 0;
	}else{
		return (myatan2(vy,vx)+RIGHT_ANGLE);
	}
}
//ベクトル成分からラジアンに変換2
double math::vec2rad2(const double& vx, const double& vy){
	if(vx==0&&vy==0){
		return 0;
	}else{
		return myatan2(vy,vx);
	}
}

//2つの座標からサインを返す
double math::getsin(const double& FromX,const double& FromY,const double& ToX,const double& ToY){
//	double oblique;
//	oblique=sqrt(pow((FromX-ToX),2)+pow((FromY-ToY),2));
//	return (ToY-FromY)/oblique;
	double rad=vec2rad2(ToX-FromX,ToY-FromY);
	return mysin(rad);
}

//2つの座標からコサインを返す
double math::getcos(const double& FromX,const double& FromY,const double& ToX,const double& ToY){
//	double oblique;
//	oblique=sqrt(pow((FromX-ToX),2)+pow((FromY-ToY),2));
//	return (ToX-FromX)/oblique;
	double rad=vec2rad2(ToX-FromX,ToY-FromY);
	return mycos(rad);
}
//mからnまでの整数をランダムで返す
int math::irand(int m,int n){
	return GetRand(n-m)+m;
}

//2つの座標から距離
double math::GetDist(const double &FromX, const double &FromY, const double &ToX, const double &ToY){
	double oblique;
	oblique=sqrt(pow((FromX-ToX),2)+pow((FromY-ToY),2));
	return oblique;
}
...
//2つの座標から距離(若干高速版)
double math::GetDist2(const double &FromX, const double &FromY, const double &ToX, const double &ToY){
	double oblique;
	oblique=pow((FromX-ToX),2)+pow((FromY-ToY),2);
	return oblique;
}